...
const MyBookings = ({ token }) => {
  ...
  const load = () => {
    setLoading(true);
    request('GET', '/api/bookings/my-bookings', null, token)
      .then(setBookings)
      .catch(() => {})
    ...
    <div>
      ...
      {bookings.length === 0 ? (
        <div className="empty">
          ...
          <div className="empty-sub">Browse resources to make your first booking</div>
        </div>
      ) : (
                  ...
                  <tr key={b._id}>
                    ...
                    <td>
                      {b.finalStatus === 'pending' && (
                        <button className="btn btn-danger btn-sm btn-icon" title="Cancel" onClick={() => cancel(b._id)}>
